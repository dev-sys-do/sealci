mod config;
mod event_listener;
mod controller;

use crate::config::Config;
use crate::event_listener::listen_to_commits;
use crate::controller::send_to_controller;
use clap::{Arg, Command};
use std::path::Path;
use std::sync::Arc;
use tokio::runtime::Runtime;


fn main() {
    // CLI arguments
    let matches = Command::new("GitHub Monitor")
        .version("1.0")
        .about("Monitors a GitHub repository for changes")
        .arg(Arg::new("config")
            .short('c')
            .long("config")
            .required(false)
            .help("The path to the config file"))
        .arg(Arg::new("repo_owner")
            .long("owner")
            .required(false)
            .help("The owner of the repo to watch"))
        .arg(Arg::new("repo_name")
            .long("repo")
            .required(false)
            .help("The name of the repo to watch"))
        .arg(Arg::new("github_token")
            .long("token")
            .required(false)
            .help("The GitHub token"))
        .arg(Arg::new("pipeline")
            .long("pipeline")
            .required(false)
            .help("The path to the actions file"))
        .get_matches();

    let config: Config = if let Some(config_path) = matches.get_one::<String>("config") {
        println!("-- SealCI - Loading config from file: {:?}", config_path);
        Config::from_file(config_path)
    } else {
        println!("-- SealCI - Loading config from CLI arguments");
        Config {
            repo_owner: matches.get_one::<String>("repo_owner").expect("--owner argument is required").to_string(),
            repo_name: matches.get_one::<String>("repo_name").expect("--repo argument is required").to_string(),
            github_token: matches.get_one::<String>("github_token").expect("--token argument is required").to_string(),
            actions_path: matches.get_one::<String>("pipeline").expect("--pipeline argument is required").to_string()
        }
    };

    println!("-- SealCI - Config loaded !");
    println!("{:#?}", config);

    // Emprunter `config` par référence
    let config: Arc<Config> = Arc::new(config);

    // Initialisation du runtime en dehors de la fermeture
    let rt: Runtime = Runtime::new().expect("Failed to create runtime");

    listen_to_commits(&config.repo_owner, &config.repo_name, &config.github_token, {
        let config: Arc<Config> = Arc::clone(&config);
        let repo_url: String = format!("https://github.com/repos/{}/{}", config.repo_owner, config.repo_name);
        
        move || {
            rt.block_on(async {
                match send_to_controller("pipeline_name", &repo_url, Path::new(&config.actions_path)).await {
                    Ok(_) => println!("Pipeline sent successfully"),
                    Err(e) => eprintln!("Failed to send pipeline: {}", e),
                }
            });
        }
    });
}
